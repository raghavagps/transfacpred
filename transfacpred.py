...
parser.add_argument("-m","--model",type=int, choices = [1, 2], help="Model: 1: ProtBert based approach, 2: Hybrid (CNN+BLAST)  approach, by default 1")
...
class TranscriptionFactor(torch.utils.data.Dataset):
...
if Model==1:
    print("======Initiating Prediction Using ProtBert Model. Please Wait.......================")
    ...
    for i in seq:
        sequence = i.upper()
        sequence = re.sub(r"[BJOUZ]", "X", sequence)
        sequence = ' '.join(sequence)
        trainer = Trainer(model = model)
        encodings = tokenizer([sequence], truncation=True, padding = True, max_length = 512)
        no_of_sequences = 1
        Y = np.zeros(no_of_sequences, dtype = 'int')
        dataset = TranscriptionFactor(encodings, Y)
        predictions, labels, metrics = trainer.predict(test_dataset=dataset)
        cc.append(predictions)
    y_pred = []
    no_of_sequences = len(seq)
    for idx in range(no_of_sequences):
        y_pred.append(np.argmax(cc[idx]))
    sam = []
    for idx in range(no_of_sequences):
        if y_pred[idx] == 1:
            sam.append("Transcription Factor")
        else:
            sam.append("Non-Transcription Factor")
    df2 = pd.DataFrame(sam)
    df4 = pd.concat([df_2,df2],axis=1)
    df4.columns = ['Seq_ID','Prediction']
    if dplay == 1:
        df4 = df4.loc[df4.Prediction=="Transcription Factor"]
    else:
        df4 = df4
